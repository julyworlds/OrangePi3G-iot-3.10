...
$(BOOTLOADER_OUT)/build-$(PROJECT)/lk_header.bin: $(OUTELF)
	...
	../../external/mediatek/build/tools/mkimage $(BOOTLOADER_OUT)/build-$(PROJECT)/lk-no-mtk-header.bin LK > $(BOOTLOADER_OUT)/build-$(PROJECT)/lk_header.bin
...
$(BOOTLOADER_OUT)/build-$(PROJECT)/merge_pre.bin: ../../../mediatek/preloader/Download/flash/preloader_$(PROJECT).bin_sram ../../../mediatek/preloader/Download/flash/preloader_$(PROJECT).bin_mem
	cat ../../external/mediatek/preloader/Download/flash/preloader_$(PROJECT).bin_sram ../../external/mediatek/preloader/Download/flash/preloader_$(PROJECT).bin_mem > $@
...
$(OUTBIN): $(OUTELF)
	...
	../../external/mediatek/build/tools/mkimage $@ LK > $(BOOTLOADER_OUT)/build-$(PROJECT)/lk_header.bin
...
ifeq ($(BUILD_SEC_LIB),yes)
	...
	@echo linking $@ $(HCTOBJS)
	$(NOECHO)$(LD) $(LDFLAGS) -T $(LINKER_SCRIPT) $(OUTPUT_TZ_BIN) $(ALLOBJS) $(HCTOBJS) $(LIBGCC) $(LIBSEC) $(LIBSEC_PLAT) -o $@
else
...
ifeq ($(BUILD_SEC_LIB),yes)
	...
	@echo linking $@ $(HCTOBJS)
	$(NOECHO)$(LD) $(LDFLAGS) -T $(LINKER_SCRIPT) $(ALLOBJS) $(HCTOBJS) $(LIBGCC) $(LIBSEC) $(LIBSEC_PLAT) -o $@
endif
