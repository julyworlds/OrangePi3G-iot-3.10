...
////////////////////////////////////////////////////////////////////////////////
struct mt6323_ac_data {
    struct power_supply psy;
    int AC_ONLINE;    
};

struct mt6323_usb_data {
    struct power_supply psy;
    int USB_ONLINE;    
};

struct mt6323_battery_data {
    struct power_supply psy;
    int BAT_STATUS;
    int BAT_HEALTH;
    int BAT_PRESENT;
...
static enum power_supply_property mt6323_ac_props[] = {
    POWER_SUPPLY_PROP_ONLINE,
};

static enum power_supply_property mt6323_usb_props[] = {
    POWER_SUPPLY_PROP_ONLINE,
};
...
static int mt6323_ac_get_property(struct power_supply *psy,
    enum power_supply_property psp,
    union power_supply_propval *val)
{
    int ret = 0;
    struct mt6323_ac_data *data = container_of(psy, struct mt6323_ac_data, psy);    
...
static int mt6323_usb_get_property(struct power_supply *psy,
    enum power_supply_property psp,
    union power_supply_propval *val)
{
    int ret = 0;
    struct mt6323_usb_data *data = container_of(psy, struct mt6323_usb_data, psy);    
...
{
    int ret = 0;     
    struct mt6323_battery_data *data = container_of(psy, struct mt6323_battery_data, psy);
...
/* mt6323_ac_data initialization */
static struct mt6323_ac_data mt6323_ac_main = {
    .psy = {
    .name = "ac",
    .type = POWER_SUPPLY_TYPE_MAINS,
    .properties = mt6323_ac_props,
    .num_properties = ARRAY_SIZE(mt6323_ac_props),
    .get_property = mt6323_ac_get_property,                
    },
    .AC_ONLINE = 0,
};

/* mt6323_usb_data initialization */
static struct mt6323_usb_data mt6323_usb_main = {
    .psy = {
    .name = "usb",
    .type = POWER_SUPPLY_TYPE_USB,
    .properties = mt6323_usb_props,
    .num_properties = ARRAY_SIZE(mt6323_usb_props),
    .get_property = mt6323_usb_get_property,                
    },
    .USB_ONLINE = 0,
};

/* mt6323_battery_data initialization */
static struct mt6323_battery_data mt6323_battery_main = {
    .psy = {
    .name = "battery",
...
#if defined(CONFIG_POWER_EXT)
#else
static void mt6323_ac_update(struct mt6323_ac_data *ac_data)
{
...
static void mt6323_usb_update(struct mt6323_usb_data *usb_data)
{
...
static void mt6323_battery_update(struct mt6323_battery_data *bat_data)
{
...
static void mt6323_battery_update_power_down(struct mt6323_battery_data *bat_data)
{
    struct power_supply *bat_psy = &bat_data->psy;

    bat_data->BAT_CAPACITY = 0;

    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[Battery] mt6323_battery_update_power_down\r\n");
...
static int mt6323_battery_probe(struct platform_device *dev)    
{
    struct class_device *class_dev = NULL;
    int ret=0;
    int i=0;

    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6323 battery driver probe!! ********\n" );
        
    /* Integrate with Android Battery Service */
    ret = power_supply_register(&(dev->dev), &mt6323_ac_main.psy);
    if (ret)
    {            
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register AC Fail !!\n");                    
        return ret;
    }             
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register AC Success !!\n");

    ret = power_supply_register(&(dev->dev), &mt6323_usb_main.psy);
    if (ret)
    {            
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register USB Fail !!\n");                    
        return ret;
    }             
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register USB Success !!\n");

    ret = power_supply_register(&(dev->dev), &mt6323_battery_main.psy);
    if (ret)
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register Battery Fail !!\n");
        return ret;
    }
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] power_supply_register Battery Success !!\n");
    ...
    class_dev = (struct class_device *)device_create(adc_cali_class, 
                                                   ...
                                                   NULL, 
                                                   ADC_CALI_DEVNAME);
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6323 BAT_probe] adc_cali prepare : done !!\n ");    
...
{
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[MT6323 BAT_probe] ChargerHwInit\n" );
    }
...
void BAT_thread(void)
{
    ...
    if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        {
            #if defined(CONFIG_POWER_EXT)
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] CONFIG_POWER_EXT, no update mt6323_battery_update_power_down.\n");
            #else
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery] Tbat(%d)>=60, system need power down.\n", BMT_status.temperature);

            mt6323_battery_update_power_down(&mt6323_battery_main);
	...
	if(gFG_booting_counter_I_FLAG == 1)
	{
		...
		else
		{
			...
			{			
				mt6323_ac_update(&mt6323_ac_main);
				mt6323_usb_update(&mt6323_usb_main);
				mt6323_battery_update(&mt6323_battery_main);  
			}
    ...
    else if(gFG_booting_counter_I_FLAG == 1)
    {
        ...
        {
            mt6323_battery_main.BAT_CAPACITY = 50;
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {               
        ...
        if (Enable_BATDRV_LOG == 1) {
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6323 battery : bat_thread_kthread : 1 ********\n" );
        }

        wait_event(bat_thread_wq, bat_thread_timeout);

        if (Enable_BATDRV_LOG == 1) {
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6323 battery : bat_thread_kthread : 2 ********\n" );
        }
...
{
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6323 battery : bat_thread_wakeup : 1 ********\n" );
    }
    ...
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6323 battery : bat_thread_wakeup : 2 ********\n" );
    }    
}
///////////////////////////////////////////////////////////////////////////////////////////
//// fop API 
...
static int mt6323_battery_probe(struct platform_device *dev)    
{
    ...
    //xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[mt6323_battery_probe] BATON_TDET_EN=1 & RG_BUF_PWD_B=1\n");
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[mt6323_battery_probe] bat_thread_kthread Done\n");    
...
static int mt6323_battery_remove(struct platform_device *dev)    
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6323 battery driver remove!! ********\n" );

    return 0;
}

static void mt6323_battery_shutdown(struct platform_device *dev)    
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6323 battery driver shutdown!! ********\n" );

}

static int mt6323_battery_suspend(struct platform_device *dev, pm_message_t state)    
{
    //xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6323 battery driver suspend!! ********\n" );

    return 0;
}

static int mt6323_battery_resume(struct platform_device *dev)
{
    //xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6323 battery driver resume!! ********\n" );
...
struct platform_device MT6323_battery_device = {
    .name   = "mt6323-battery",
    .id        = -1,
};
...
static struct platform_driver mt6323_battery_driver = {
    ...
    .remove        = mt6323_battery_remove,
    .shutdown      = mt6323_battery_shutdown,
    //#ifdef CONFIG_PM
    .suspend       = mt6323_battery_suspend,
    .resume        = mt6323_battery_resume,
    //#endif
    .driver        = {
        .name = "mt6323-battery",
    },
...
static int __init mt6323_battery_init(void)
{
    int ret;

    ret = platform_device_register(&MT6323_battery_device);
    if (ret) {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_battery_driver] Unable to device register(%d)\n", ret);
    return ret;
    }
    
    ret = platform_driver_register(&mt6323_battery_driver);
    if (ret) {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_battery_driver] Unable to register driver (%d)\n", ret);
    return ret;
    }
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_battery_driver] Initialization : DONE !!\n");
    return 0;
}

static void __exit mt6323_battery_exit (void)
{
}

module_init(mt6323_battery_init);
module_exit(mt6323_battery_exit);

MODULE_AUTHOR("James Lo");
MODULE_DESCRIPTION("MT6323 Battery Device Driver");
MODULE_LICENSE("GPL");
