...
void mtkfb_pan_disp_test(void)
{
    ...
    sem_flipping_cnt--;
    pr_warn("DISP/MTKFB" "wait sem_flipping\n");
    ...
    pr_warn("DISP/MTKFB" "wait sem_early_suspend\n");
    ...
    pr_warn("DISP/MTKFB" "wait sem_overlay_buffer\n");
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    int i;
    disp_job *job;
    static unsigned long int last_primary_trigger_time = 0;
    unsigned long int trigger_time_duration = 0;
    ...
    {
        // Need delay frame update due to frame would be dropped if the frame update period is too closed.
        trigger_time_duration = get_current_time_us() - last_primary_trigger_time;
        if(trigger_time_duration < 16667)
        {
            unsigned int delay;
            delay = (unsigned int)((16667 - trigger_time_duration)/1000 + 1);
            MTKFB_WRAN("Trigger frame update too close, delay:%d msec\n", delay);
            msleep(delay);
        }
        last_primary_trigger_time = get_current_time_us();
...
#endif
{
    ...
    {
        MTKFB_IOCTL("[%s] MTKFB_CONFIG_IMMEDIATE_UPDATE, enable = %lu\n",
            __func__, arg);
		if (down_interruptible(&sem_early_suspend)) {
    ...
    case MTKFB_GET_OVERLAY_LAYER_INFO:
    {
        ...
        MTKFB_IOCTL(" mtkfb_ioctl():MTKFB_GET_OVERLAY_LAYER_INFO\n");
    ...
    {
        struct fb_overlay_layer layerInfo;
        MTKFB_IOCTL(" mtkfb_ioctl():MTKFB_SET_OVERLAY_LAYER\n");
    ...
    {
        MTKFB_IOCTL("[DDP] mtkfb_ioctl():MTKFB_ERROR_INDEX_UPDATE_TIMEOUT  \n");
    ...
    case MTKFB_SET_VIDEO_LAYERS:
    {
        ...
        MTKFB_IOCTL(" mtkfb_ioctl():MTKFB_SET_VIDEO_LAYERS\n");
    ...
    case MTKFB_SET_MULTIPLE_LAYERS:
    {
        ...
        MTKFB_IOCTL(" mtkfb_ioctl():MTKFB_SET_MULTIPLE_LAYERS\n");
    ...
    {
        MTKFB_IOCTL("[MTKFB] Set Orientation: %lu\n", arg);
	...
	case MTKFB_GET_INTERFACE_TYPE:
	{
		...
		MTKFB_IOCTL("[MTKFB] MTKFB_GET_INTERFACE_TYPE\n");
	...
	{
	    unsigned int speed;
		MTKFB_IOCTL("[MTKFB] get default update speed\n");
	...
	{
	    unsigned int speed;
		MTKFB_IOCTL("[MTKFB] get current update speed\n");
	...
	{
	    unsigned int speed;
		MTKFB_IOCTL("[MTKFB] change update speed\n");
	...
	case MTKFB_FACTORY_AUTO_TEST:
	{
		...
		MTKFB_IOCTL("factory mode: lcm auto test\n");
	...
	{		
        ...
        MTKFB_IOCTL("MTKFB_SLT_AUTO_CAPTURE\n");
...
int m4u_reclaim_mva_callback_ovl(int moduleID, unsigned int va, unsigned int size, unsigned int mva)
{
    ...
    MTKFB_INFO("Warning: m4u_reclaim_mva_callback_ovl. mva=0x%08X size=0x%X dirty=%d\n", mva, size, dirty);
    if (dirty)
    {
...
static void mtkfb_fb_565_to_8888(struct fb_info *fb_info)
{
    ...
    MTKFB_INFO("mtkfb_fb_565_to_8888 xres=%d yres=%d fbsize=0x%X x_virtual=%d s=0x%p d=0x%p\n",
...
static void mtkfb_fb_565_to_888(void* fb_va)
{
    ...
    MTKFB_INFO("565_to_888, s = 0x%p, d=0x%p\n", s,d);
    for (j = 0; j < yres; ++ j)
    {
        for(k = 0; k < xres; ++ k)
        {
...
static int mtkfb_probe(struct device *dev)
{
    ...
    {
        lcd_fps = 6000;
        MTKFB_WRAN("[FB driver]can not get fps from uboot\n");
    }
    else
    {
        p += 4;
