...
DCP_STATUS disp_set_session_input (struct disp_session_input_config_t *input) {
	...
	for (i = 0; i < MAX_INPUT_CONFIG; i++) {
		...
		if (config->layer_id >= MAX_INPUT_CONFIG) {
			//make sure layer.dirty is false
			XLOG_WARN("disp_set_session_input: found invalid layer_id=%x(%d), mark un-dirty layer\n", config->layer_id, config->layer_id);
			job->input[i].dirty = 0;
			continue;
		}
		if (config->layer_enable) {
			if (config->next_buff_idx <= disp_sync_get_last_signaled(
				...
				XLOG_WARN("L%d of 0x%08x is enabled, but HWC set an old buffer(%d)!\n",
						config->layer_id, input->session_id, config->next_buff_idx);

				/* Disable the layer since the source buffer would be released. */
			...
			case DISP_FORMAT_XRGB8888:
				job->input[i].format = eXARGB8888;
				layerpitch = 4;
				layerbpp = 32;
				break;
			case DISP_FORMAT_XBGR8888:
				job->input[i].format = eXABGR8888;
				layerpitch = 4;
				layerbpp = 32;
				break;
		    ...
		    job->input[i].layer_enable = (config->layer_enable != 0) ? 1 : 0;
		    job->input[i].dirty = 1;
		...
		XLOG_DBG("disp_set_session_input L%d, e%d, d%d\n", job->input[i].layer_id, job->input[i].layer_enable, job->input[i].dirty);
	}
...
DCP_STATUS disp_set_session_output (struct disp_session_output_config_t *output) {
	...
	mutex_unlock(&job->lock);
	XLOG_DBG("L%d output:%d,0x%x\n", job->output.layer_id, job->output.index, job->output.address);
	return ret;
}
