...
static void _DPI_LogRefreshRate(DPI_REG_INTERRUPT status)
{
   ...
   if (status.VSYNC)
   {
      ...
      {
         pr_info("DISP/DPI" "Receive 1 vsync in %lu us\n", 
                                      curr.tv_usec - prevUs);
      }
      prevUs = curr.tv_usec;
   }
