...
static DISP_STATUS dpi_init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
...
#else
    {
        ...
        config.ovl_config.fmt = ePARGB8888;
        ...
        config.ovl_config.src_pitch = ALIGN_TO(DISP_GetScreenWidth(),32)*4; //pixel number
...
static DISP_STATUS dpi_update_screen(BOOL isMuextLocked)
{
    ...
    if (DDMS_capturing)
        pr_info("DPI" "[DISP] kernel - dpi_update_screen. DDMS is capturing. Skip one frame. \n");		
