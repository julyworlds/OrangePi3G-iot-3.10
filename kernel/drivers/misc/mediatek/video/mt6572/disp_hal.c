...
int disphal_process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "te:", 3))
    {
        if (0 == strncmp(opt + 3, "on", 2)) {
            if (DSI_Get_EXT_TE())
            {
                //pr_info("DISP/DSI" "EXT TE is enabled, can not enable BTA TE now\n");
            }
            else
            {
                //pr_info("DISP/DSI" "Before: BTA_TE = %d, EXT_TE = %d\n", DSI_Get_BTA_TE(),DSI_Get_EXT_TE());
                LCD_TE_Enable(TRUE);
                DSI_TE_Enable(TRUE);
                //pr_info("DISP/DSI" "After : BTA_TE = %d, EXT_TE = %d\n", DSI_Get_BTA_TE(),DSI_Get_EXT_TE());
            }
    ...
    else if (0 == strncmp(opt, "te_ext:", 7))
    {
        if (0 == strncmp(opt + 7, "on", 2)) {
            if (DSI_Get_BTA_TE())
            {
                //pr_info("DISP/DSI" "BTA TE is enabled, can not enable EXT TE now\n");
            }
            else
            {
                //pr_info("DISP/DSI" "Before: BTA_TE = %d, EXT_TE = %d\n", DSI_Get_BTA_TE(),DSI_Get_EXT_TE());
