...
#define DISP_DRV_INFO(fmt, arg...) \
    do { \
        if (disp_drv_dbg_info_log) pr_info("DISP" fmt, ##arg); \
    }while (0)

#define DISP_DRV_FUNC()	\
    do { \
        if(disp_drv_dbg_func_log) pr_info("DISP" "[Func]%s\n", __func__); \
    }while (0)

#define DISP_DRV_UPD(fmt, arg...) \
    do { \
        if (disp_drv_dbg_upd_log) pr_info("DISP" fmt, ##arg); \
    }while (0)
...
#define MTKFB_INFO(fmt, arg...) \
    do { \
        if (mtkfb_dbg_log) pr_info("MTKFB" fmt, ##arg); \
    }while (0)

#define MTKFB_FENCE(fmt, arg...) \
    do { \
        if (mtkfb_dbg_fence_log) pr_info("MTKFB" fmt, ##arg); \
    }while (0)

#define MTKFB_FUNC()	\
    do { \
        if(mtkfb_dbg_func_log) pr_info("MTKFB" "[Func]%s\n", __func__); \
    }while (0)

#define MTKFB_IOCTL(fmt, arg...) \
    do { \
        if (mtkfb_dbg_ioctl_log) pr_info("MTKFB" fmt, ##arg); \
    }while (0)
