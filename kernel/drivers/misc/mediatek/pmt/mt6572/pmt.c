...
int part_num_emmc = 0;
int MBR_EBR_p;
unsigned char part_name_emmc[PART_MAX_COUNT][MAX_PARTITION_NAME_LEN];
...
{
	u8 padding[446];
	struct mbr_partition part_table[4];
	u8 sig1;
	u8 sig2;
};
...
static struct pmt_region mbr_regions[] = {
	{
			.base_addr = 0,
			.size = PMT_REGION_SIZE,
			.sig = MBR_SIG,
...
static int __load_pmt(struct pmt_region *region, pt_resident *part)
{
	...
	pmt_info("__load_pmt region 0x%llx\n",region->base_addr);
...
static unsigned long long parsing_mbr(u32 start_address, u32 index)
{
	int i,j, k;
	unsigned long long offset[4];
	unsigned long long	ebr_address = 0xFFFFFFFF;
	int err;
	char temp_buf[512];
	u32 temp_part[4];
	for(i = 0; i<4;i++)
	{
		memcpy(temp_part, &mbr_buffer[446+i*16],16); //read partition
		if(temp_part[2] == 0)
			break;
		offset[i] = (unsigned long long) temp_part[2]*512 + start_address;
		pmt_info("offset[%d] 0x%x 0x%llx\n",i,temp_part[2], offset[i]);
		
		err = eMMC_rw_x(offset[i], (unsigned int *)&temp_buf, 0, 0, 512, 1, USER);
		if (temp_buf[510] == 0x55 && temp_buf[511] == 0xAA) 
		{
			if(!memcmp(mbr_buffer, temp_buf, 446)) //avoid FAT
			{
				ebr_address = offset[i];
				pmt_info("next ebr address 0x%llx\n",ebr_address);
			}
		}
	}
	k = 0;
	for(j = 0; j < PART_MAX_COUNT; j++)
	{
		//pmt_info("last_part[%d].offset(0x%llx) (offset[%d]+MBR_START_ADDRESS_BYTE)(0x%llx))",j,last_part[j].offset,k,(offset[k]+MBR_START_ADDRESS_BYTE));
		if(last_part[j].offset == (offset[k]+MBR_START_ADDRESS_BYTE))
		{
			MBR_EBR_px[index].part_index[k] = MBR_EBR_p;
			PartInfo[j].partition_idx = MBR_EBR_p;
			
			pmt_info("xbr record %d %d %s\n",MBR_EBR_px[index].part_index[k], PartInfo[j].partition_idx,PartInfo[j].name);
			MBR_EBR_p++;
			k++;
			i--;
		}
		if(0 == i || 0 == last_part[j].size)
			break;
	}
	return ebr_address;
}

static int construct_mbr_mapping()
{
	int err = 0;
	int i=0,j;
	u64 offset[4];
	unsigned long long xbr_address = 0;
	MBR_EBR_p = 1;
	while(0xFFFFFFFF != xbr_address){
		pmt_info("read xbr address 0x%llx\n",xbr_address);
		err = eMMC_rw_x(xbr_address, (unsigned int *)mbr_buffer, 0, 0, 512, 1, USER);
		if (err || mbr_buffer[510] != 0x55 || mbr_buffer[511] != 0xAA) {
			pmt_info("read mbr error %d 0x%x 0x%x\n",err,mbr_buffer[510],mbr_buffer[511]);
	        err = -EIO;
			goto out;
		}
		xbr_address = parsing_mbr(xbr_address,i); //mbr
		i++;
	}
    ...
    if (err) { 
	   ...
	   goto fail_malloc;
        ...
        for (i = 0; i < PART_MAX_COUNT; i++) {
			pmt_info("%s, start(0x%16llx) - size(0x%16llx)\n",last_part[i].name,last_part[i].offset,last_part[i].size);
			if(last_part[i].size == 0)
				break;
			for(j=0; j < MAX_PARTITION_NAME_LEN; j++)
			{
				if(last_part[i].name[j] == 0)
					break;
				part_name_emmc[i][j] = lowercase(last_part[i].name[j]);
			}
			PartInfo[i].name = part_name_emmc[i];
			...
			//if(!strcmp(PartInfo[i].name,"fat"))  //resize fat partition
			//{
			//	pmt_info("fat resize\n");
			//	PartInfo[i].size = user_region_size_byte - PartInfo[i].start_address - (64*1024*1024/512); //64MB reserve
			//}
        }
		part_num_emmc = i;
    }
	construct_mbr_mapping();
	//pmt_info("part_num_emmc %d\n",
    //            part_num_emmc);

    for (i = 0; i < part_num_emmc; i++) {  
        pmt_info("Partition[%2d](%-16s) - start(0x%16llx) - size(0x%16llx) - partition_idx (%d)\n",
                i, PartInfo[i].name, PartInfo[i].start_address, PartInfo[i].size,PartInfo[i].partition_idx);
    }
	pmt_info("init_pmd done\n");
	pmt_done = 1;
	err = 0;
...
int get_part_num_emmc()
{
	return part_num_emmc;
}

EXPORT_SYMBOL(get_part_num_emmc);
