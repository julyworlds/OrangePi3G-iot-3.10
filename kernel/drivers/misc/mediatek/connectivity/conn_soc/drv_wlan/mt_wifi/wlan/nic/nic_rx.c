	...
	case EVENT_ID_GSCAN_RESULT:
		{
			...
			if (g_GetResultsCmdCnt == 0) {
				DBGLOG(SCN, INFO,
					("FW report events more than the wifi_hal asked number,
					buffer the results for future reporting\n"));
				UINT_8 i = 0;
	...
	do {
		...
		if (prSwRfb) {
			...
			case HIF_RX_PKT_TYPE_TX_LOOPBACK:
				...
				{
					extern void
					    kalDevLoopbkRxHandle(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb);
