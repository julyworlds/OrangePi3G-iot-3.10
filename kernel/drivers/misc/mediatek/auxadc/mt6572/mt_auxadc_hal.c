...
int IMM_auxadc_GetOneChannelValue(int dwChannel, int data[4], int* rawdata)
{
   ...
   if(clock_is_on(MT_PDN_PERI_AUXADC) == PWR_DOWN)
   {
   ...
   /* Modified for AUXADC_XP/AUXADC_YM as general ADC usage */
   if((dwChannel == PAD_AUX_YM) || (dwChannel == PAD_AUX_XP) )
   {
        printk("[adc_api]: channel %d : disable penirq\n",dwChannel );
