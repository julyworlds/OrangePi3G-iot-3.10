...
bool usb_phy_check_in_uart_mode()
{
	...
	UINT8 usb_port_mode = USB_PHY_Read_Register8(0x6B);
...
void usb_phy_switch_to_uart(void)
{
  ...
  printk("usb port value in uart function:%d\n", usb_port_mode_temp);
	...
	/* GPIO Selection */
#if 0
	DRV_WriteReg32(GPIO_BASE + 0x504, 0x10);	//set UART1
#else
	DRV_WriteReg32(GPIO_BASE + 0x504, 0x20);	//set UART2
#endif
}
...
void usb_phy_switch_to_usb(void)
{
	/* GPIO Selection */
#if 0
	DRV_WriteReg32(GPIO_BASE + 0x508, 0x10);	//clear UART1
#else
	DRV_WriteReg32(GPIO_BASE + 0x508, 0x20);	//clear UART2
#endif
