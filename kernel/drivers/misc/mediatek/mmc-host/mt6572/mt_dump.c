...
#include <mach/mt_clkmgr.h>  //for enable_clock()
...
#endif
char test_kdump[]={6,5,7,2,'k','d','u','m','p','t','e','s','t'};
//==============
#if MTK_MMC_DUMP_DBG
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
    ...
    if (sdr_read32(0xF0000000) & 0x80)
    {
        sdr_write32(0xF0205124, 0x800F6000);
    }
    else // DDR3
    {
    ...
    //For bbchip without MSDCPLL, MSDC source is divided from other PLL, e.g., UPLL, MPLL, and etc.
    //The related cg bits of source PLL shall also be un-gated
    // since they may be occasionally or intentionally gated.
    sdr_write32(SET_MPLL_FREDIV_EN, CG_MPLL_MASK);
    mdelay(1); //delay at least 30~40 us for 6571

    // Turn off MSDC0/1 clock by set CG
    sdr_write32(0xF0000054, 0x00060000);
    // Set clock source index=2 to as select MAIN PLL/8.
    sdr_write32(0xF0000000, (DRV_Reg32(0xF0000000) & 0xFF8FC7FF) | 0x00201000);
    // Turn on MSDC0/1 clock by clear CG
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    switch (host->id) {
        case 0:
            sdr_set_field(MSDC0_PAD_CTRL_PULLSEL_CFG, MSDC0_PAD_PULLSEL_ALL_MASK, 0x3FE);
            ...
            break;
        case 1:
            sdr_set_field(MSDC1_PAD_CTRL_PULLSEL_CFG, MSDC1_PAD_PULLSEL_ALL_MASK, 0x3E );
...
static unsigned int simple_mmc_attach_mmc(struct simp_mmc_host *host)
{
    ...
    /* power up host */
    //simp_mmc_power_up(host,0);    //Removed according to ALPS01184362
    //mdelay(20);                   //Removed according to ALPS01184362
    simp_mmc_power_up(host,1);
    mdelay(10);                     //Added according to ALPS01184362
...
out:
    if ( err ) simp_msdc_dump_register(host);
    return err;
}
...
static int simp_emmc_dump_write(unsigned char* buf, unsigned int len, unsigned int offset,unsigned int dev)
{
    ...
    if ( (PartInfo[l_dest_num].size < len) ||
         (PartInfo[l_dest_num].size < offset) ||
    ...
    for (i = 0; i < (len/512); i++) {
        ...
        if ( l_addr >= ((PartInfo[l_dest_num].start_address + PartInfo[l_dest_num].size) >>9 ) ) {
