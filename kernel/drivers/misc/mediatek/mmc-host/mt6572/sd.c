...
static void msdc_dump_register(struct msdc_host *host)
{
    ...
    INIT_MSG("Reg[A8] DMA_LEN        = 0x%.8x", sdr_read32(base + 0xA8));
    ...
    switch(host->id){
        ...
        case 1:
            if(sd_18){
                ...
                sdr_set_field(MSDC1_PAD_CTRL_RDSEL_CFG, MSDC1_RDSEL_MASK ,0xC);
            }
            break;
        default:
            break;
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
    ...
    if (mmc_card_sd(card) && (host->hw->host_function == MSDC_SD)) {
        ...
        if((host->power_cycle < MSDC_MAX_POWER_CYCLE) && (host->power_cycle_enable))
        {
            ...
            sdr_get_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_BUSY_MA, host->saved_para.write_busy_margin);
            sdr_get_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_CRC_MA, host->saved_para.write_crc_margin);
...
static void msdc_restore_emmc_setting(struct msdc_host *host)
{
    ...
    sdr_set_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_BUSY_MA, host->saved_para.write_busy_margin);
    sdr_set_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_CRC_MA, host->saved_para.write_crc_margin);
}
...
static struct tag_pt_info* msdc_reserve_part_info(unsigned char* name)
{
#if 0
    int i;
...
#else
	if (0 == strcmp(name, "bmtpool"))
		return &bmt_info;
	if (0 == strcmp(name, "otp"))
		return &otp_info;
#endif
    return NULL;
}
...
{
    u32 l_offset;
    struct tag_pt_info* lp_excel_info;
    u32 l_otp_reserve = 0;
    u32 l_mtk_reserve = 0;
...
printk("otp reserve: start address = %llu\n", lp_excel_info->start_address);//========================debug
	...
	if ((l_offset + l_otp_reserve + l_mtk_reserve) == 0xF00)
		return (l_offset + l_otp_reserve + l_mtk_reserve)+BMT_POOL_SIZE;
...
/* The abort condition when PIO read/write
   tmo:
*/
int msdc_pio_abort(struct msdc_host *host, struct mmc_data *data, unsigned long tmo)
{
    int  ret = 0;
    u32  base = host->base;
...
#ifdef CONFIG_MTK_TC1_FEATURE
        if(host->hw->host_function == MSDC_SDMMC) {
            host->card_inserted = 0;
            if(host->mmc->card){
                spin_lock_irqsave(&host->remove_bad_card,flags);
                got_polarity = host->sd_cd_polarity;
...
#ifdef CONFIG_MTK_TC1_FEATURE
    if( (host->t_counter.time_cmd % CMD_TUNE_HS_MAX_TIME) == 0 )
    {
        ERR_MSG(">> try power tunning before cmd latch time tune");
        msdc_power_tuning(host);
    }
#endif /* CONFIG_MTK_TC1_FEATURE */
...
#ifdef CONFIG_MTK_TC1_FEATURE
    if( (host->t_counter.time_read % READ_TUNE_HS_MAX_TIME) == 0 )
    {
        ERR_MSG(">> try power tunning before read latch time tune");
        msdc_power_tuning(host);
    }
#endif /* CONFIG_MTK_TC1_FEATURE */
...
#ifdef CONFIG_MTK_TC1_FEATURE
    if( (host->t_counter.time_write % WRITE_TUNE_HS_MAX_TIME) == 0 )
    {
        ERR_MSG(">> try power tunning before write latch time tune");
        msdc_power_tuning(host);
    }
#endif /* CONFIG_MTK_TC1_FEATURE */
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if (host->mclk != ios->clock || host->ddr != ddr) { /* not change when clock Freq. not changed ddr need set clock*/
        ...
        if (ios->clock == 0) {
            ...
            sdr_set_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_WRDAT_CRCS,1);
            if ((host->hw->host_function == MSDC_EMMC) || (host->hw->host_function == MSDC_SD)){
                sdr_set_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_BUSY_MA, 1);
                sdr_set_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_CRC_MA, 1);
            }
        }
...
static int __init mt_msdc_init(void)
{
    ...
    {
        printk("[%s]: successfully create /proc/emmc\n", __func__);
    }else{
        printk("[%s]: failed to create /proc/emmc\n", __func__);
    }
    #endif /* CONFIG_MTK_EMMC_SUPPORT && CONFIG_PROC_FS */
