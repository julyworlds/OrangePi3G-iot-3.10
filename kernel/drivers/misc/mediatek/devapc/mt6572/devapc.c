...
static void set_module_apc(unsigned int module, DEVAPC_DOM domain_num , DEVAPC_ATTR permission_control)
{
    ...
    {
         pr_debug("[DEVAPC] ERROR, The setting is error, please check if domain master setting is correct or not !\n");
        return;
    }
...
static irqreturn_t devapc_violation_irq(int irq, void *dev_id)
{
    ...
    for (module_index = 0; module_index < DEVAPC_DEVICE_NUMBER; module_index += 32)
    {
        sta = readl(DEVAPC_D0_VIO_STA_0 + (module_index / 32) * 4);
         pr_debug("[DEVAPC] DEVAPC_D0_VIO_STA_%u = 0x%x\n", module_index/32, sta);
        status |= sta;
    }
    ...
    {
         pr_debug("[DEVAPC] ERROR DEVAPC_D0_VIO_STA not device apc AP/MM violation!\n");
        return IRQ_NONE;
    }
    ...
    if(r_w_violation == 1)
    {
        pr_debug("Vio Status:0x%x , Addr:0x%x , Master ID:0x%x , Dom ID:0x%x, W\n", status, dbg1, master_ID, domain_ID);
    }
    else
    {
         pr_debug("Vio Status:0x%x , Addr:0x%x , Master ID:0x%x , Dom ID:0x%x, R\n", status, dbg1, master_ID, domain_ID);
    }
...
/* Original Design */
    ...
    if ((dbg0 != 0) || (dbg1 != 0)) 
    {
        pr_debug("[DEVAPC] DBG Clear FAILED!\n");
...
static int __init devapc_init(void)
{
    int ret;
     ...
     pr_debug( "[DEVAPC] register platform device. \n");
    ...
    pr_debug( "[DEVAPC] register platform driver. \n");
...
#ifdef CONFIG_MTK_HIBERNATION
     pr_debug("[DEVAPC] register swsusp restore noirq func. \n");

    register_swsusp_restore_noirq_func(ID_M_DEVAPC, devapc_pm_restore_noirq, NULL);
#endif

     pr_debug( "[DEVAPC] module init done. \n");
