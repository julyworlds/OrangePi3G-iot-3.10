...
#define BMA222E_DEV_NAME        "BMA2XXE"
/*----------------------------------------------------------------------------*/

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id bma222E_i2c_id[] = {{BMA222E_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_bma222E={ I2C_BOARD_INFO("BMA2XXE", (BMA222E_I2C_SLAVE_WRITE_ADDR>>1))};
...
static bool isbma250e = true;   //add by ljs 
...
static void BMA222E_power(struct acc_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "BMA2XXE"))
			{
		...
		{
			if (!hwPowerDown(hw->power_id, "BMA2XXE"))
			{
...
/*
	...
	if(databuf[0]==BMA222E_FIXED_DEVID) {
		...
		printk("BMA222E_CheckDeviceID %d ERR!\n ", databuf[0]);
		return BMA222E_ERR_IDENTIFICATION;
	}
...
static int BMA222E_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(!isbma250e) sprintf(buf, "BMA222E Chip");
	else sprintf(buf, "BMA250E Chip");
	return 0;
}
/*----------------------------------------------------------------------------*/
...
static int bma222E_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = bma222e250e_get_cust_acc_hw();
...
static int bma222E_remove(void)
{
    struct acc_hw *hw = bma222e250e_get_cust_acc_hw();
...
{
	GSE_FUN();
      i2c_register_board_info(1, &i2c_bma222E, 1);
