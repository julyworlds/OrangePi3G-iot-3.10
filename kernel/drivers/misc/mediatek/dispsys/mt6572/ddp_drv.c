...
void cmdq_ion_flush(void)
{
    ...
    sys_data.cache_sync_param.kernel_handle = cmdqIONHandle;
...
int disp_dump_reg(DISP_MODULE_ENUM module)
{
        ...
        {       
            ...
            case DISP_MODULE_SMI:
                DDP_DRV_INFO("(0x00)SMI_LARB0_BASE             =0x%x \n", DISP_REG_GET(DDP_REG_BASE_SMI_LARB0              ));
                DDP_DRV_INFO("(0x600)SMI_LARB_FIFO_STAT0       =0x%x \n", DISP_REG_GET(DISP_REG_SMI_LARB_FIFO_STAT0        ));
                DDP_DRV_INFO("(0x604)SMI_LARB_FIFO_STAT1       =0x%x \n", DISP_REG_GET(DISP_REG_SMI_LARB_FIFO_STAT1        ));
                DDP_DRV_INFO("(0x610)SMI_LARB_BUS_STAT0        =0x%x \n", DISP_REG_GET(DISP_REG_SMI_LARB_BUS_STAT0         ));
                DDP_DRV_INFO("(0x614)SMI_LARB_BUS_STAT2        =0x%x \n", DISP_REG_GET(DISP_REG_SMI_LARB_BUS_STAT1         ));

                DDP_DRV_INFO("(0x00)SMI_COMMON_BASE            =0x%x \n", DISP_REG_GET(DDP_REG_BASE_SMI_COMMON		       ));
                DDP_DRV_INFO("(0x400)SMI_COMMON_DEBUG0	       =0x%x \n", DISP_REG_GET(DISP_REG_SMI_DEBUG0		           ));
                break;
...
int disp_clock_check(void)
{
    ...
    {
        DDP_DRV_ERR("clock error(ddp), CONFIG_CG_CON0=0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
        ret = -1;
    }
    ...
    {
        DDP_DRV_ERR("clock error(dsi), CONFIG_CG_CON1=0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON1));
        ret = -1;
    }
    ...
    {
        DDP_DRV_ERR("clock error(mm and axi), DISP_REG_CLK_CFG0=0x%x \n", DISP_REG_GET(DISP_REG_CLK_CFG0));
        ret = -1;
    }
    ...
    {
        DDP_DRV_ERR("clock error(pwm), DISP_REG_CLK_CFG1=0x%x \n", DISP_REG_GET(DISP_REG_CLK_CFG1));
        ret = -1;
    }
