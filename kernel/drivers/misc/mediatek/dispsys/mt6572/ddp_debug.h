...
#define DBI_DRV_WRAN(fmt, arg...) \
   do { \
      if (dbi_drv_dbg_log) pr_warn("DBI" fmt, ##arg);    \
   }while (0)

#define DBI_DRV_INFO(fmt, arg...) \
   do { \
      if (dbi_drv_dbg_log) pr_info("DBI" fmt, ##arg);    \
   }while (0)

#define DBI_DRV_FUNC(fmt, arg...) \
   do { \
      if(dbi_drv_dbg_func_log) pr_info("DBI" "[Func]%s\n", __func__);  \
   }while (0)

#define DSI_DRV_WRAN(fmt, arg...) \
   do { \
      if (dsi_drv_dbg_log) pr_warn("DSI" fmt, ##arg);    \
   }while (0)

#define DSI_DRV_INFO(fmt, arg...) \
   do { \
      if (dsi_drv_dbg_log) pr_info("DSI" fmt, ##arg);    \
   }while (0)

#define DSI_DRV_FUNC(fmt, arg...) \
   do { \
      if(dsi_drv_dbg_func_log) pr_info("DSI" "[Func]%s\n", __func__);  \
   }while (0)
