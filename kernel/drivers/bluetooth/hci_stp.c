...
#define HCI_STP_READY_TIMEOUT msecs_to_jiffies(200) /* 2 seconds */

#define BT_DEV_MAJOR 192 // never used number
static int BT_major = BT_DEV_MAJOR;
/*******************************************************************************
*                            P U B L I C   D A T A
********************************************************************************
*/
static unsigned int dbgLevel = BT_LOG_INFO;
...
#define BT_LOUD_FUNC(fmt, arg...)   if(dbgLevel >= BT_LOG_LOUD){printk(PFX "[L]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_DBG_FUNC(fmt, arg...)    if(dbgLevel >= BT_LOG_DBG){printk(PFX "[D]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_INFO_FUNC(fmt, arg...)   if(dbgLevel >= BT_LOG_INFO){printk(PFX "[I]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_WARN_FUNC(fmt, arg...)   if(dbgLevel >= BT_LOG_WARN){printk(PFX "[W]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_ERR_FUNC(fmt, arg...)    if(dbgLevel >= BT_LOG_ERR){printk(PFX "[E]%s:"   fmt, __FUNCTION__ ,##arg);}
#define BT_TRC_FUNC(f)              if(dbgLevel >= BT_LOG_LOUD){printk(PFX "[T]%s:%d\n", __FUNCTION__, __LINE__);}
...
static ssize_t file_read(char *filename, char *buf, size_t len, loff_t *offset)
{
    ...
    retLen = vfs_read(fp, buf, len, offset);
...
static ssize_t file_write(char *filename, char *buf, size_t len, loff_t *offset)
{
    ...
    retLen = vfs_write(fp, buf, len, offset);
...
void hci_stp_dev_init_rx_cb (const UINT8 *data, INT32 count)
{
    ...
    if (unlikely(!hci_get_drvdata(hdev))) {
        BT_ERR_FUNC("null hci_get_drvdata(hdev)!\n");
        return;
    }
...
static int hci_stp_flush(struct hci_dev *hdev)
{
    ...
    if (!hdev || ! hci_get_drvdata(hdev)) {
        BT_WARN_FUNC("invalid hdev(0x%p) or hci_get_drvdata(drv) data(0x%p)\n",
...
static void hci_stp_destruct(struct hci_dev *hdev)
{
    ...
    BT_DBG_FUNC("%s\n", hdev->name);
    hci_stp_tx_deinit((struct hci_stp *)hci_get_drvdata(hdev));
...
#endif
{
    ...
    BT_DBG_FUNC("hci_get_drvdata(hdev) 0x%08x\n", (int)hci_get_drvdata(hdev));
...
//    BT_DBG_FUNC("HCI_QUIRK_NO_RESET\n");
    ...
    cls = class_create(THIS_MODULE, "stpbtdrv");
    if (IS_ERR(cls)) {
       BT_ERR_FUNC("Unable to create class, err = %d\n", (int)PTR_ERR(cls));
        return 0;
    }
    device_create(cls,NULL,dev,NULL,"stpbt");
...
int mtk_hci_init(void)
{
    return hci_stp_init();
}
EXPORT_SYMBOL(mtk_hci_init);

int mtk_hci_exit(void)
{
    return hci_stp_exit();
}
EXPORT_SYMBOL(mtk_hci_exit);
