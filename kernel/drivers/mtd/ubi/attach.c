			    ...
			    int pnum)
{
	...
	err = ubi_io_read(ubi, ubi_peb_buf, pnum, ubi->leb_start,
			  ubi->leb_size);
		     ...
		     int backup_pnum, struct ubi_ainf_peb *cad_peb)
{
	...
	list_for_each_entry(old_seb, &ai->free, u.list)
		if (old_seb->pnum == source_pnum) {
			...
			err = add_to_list(ai, old_seb->pnum, old_seb->vol_id, old_seb->lnum, old_seb->ec, 1, &ai->erase);
			if (err)
				return err;
...
recovery:
	...
	for(i=0 ; i<num ; i++) {
		...
		if(source_page >= ubi->leb_start/ubi->mtd->writesize) {
			...
			memcpy((void *) (((char*)ubi_peb_buf)+(source_page*ubi->mtd->writesize)-ubi->leb_start),
...
write_error:
	...
	err = add_to_list(ai, new_seb->pnum, new_seb->vol_id, new_seb->lnum, new_seb->pnum, 1,
			&ai->corr);
	kmem_cache_free(ai->aeb_slab_cache, new_seb);
...
int ubi_backup_init_scan(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	struct ubi_vid_hdr *vid_hdr = NULL;
	struct ubi_ainf_volume *av;
	struct ubi_ainf_peb *seb, *backup_seb[2], *old_seb; //, *new_seb;
	struct rb_node *rb;
	...
	for(j=1;j>=0;j--) {
		...
		for(i=0 ; i<num ; i++) {
			...
			if(check_pattern(ubi->databuf, 0xFF, ubi->mtd->writesize)==1) {
				...
				if(old_seb != NULL)
				{
					ubi_msg("old seq %llu , blb seq %llu", old_seb->sqnum ,be64_to_cpu(p_blb_spare->sqnum));
		...
		if(corrupt == 1) {
			...
			blb_recovery_peb(ubi, ai, p_blb_spare, pnum, num, backup_pnum, candidate_peb);
			candidate_peb = NULL;
		} else if(corrupt == 2) {
			av = ubi_find_av(ai, source_vol_id);
			if (!av) {
				...
				ubi_rb_for_each_entry(rb, old_seb, &av->root, u.rb) {
					if( old_seb->pnum == source_pnum) {
						...
						if(candidate_peb != NULL) {
							...
							add_to_list(ai, candidate_peb->pnum, candidate_peb->vol_id, candidate_peb->lnum, candidate_peb->ec, 1, &ai->free);
			...
			if(candidate_peb != NULL) {
				...
				err = early_erase_peb(ubi, ai, candidate_peb->pnum, candidate_peb->ec+1);
				if (err) {
					ubi_msg("erasing candidate peb %d fail %d", candidate_peb->pnum, err);
					add_to_list(ai, old_seb->pnum, old_seb->vol_id, old_seb->lnum, old_seb->ec, 1, &ai->erase);
					kmem_cache_free(ai->aeb_slab_cache, candidate_peb);
					candidate_peb = NULL;
				}
				candidate_peb->ec++;
