			...
			bool input)
{
	...
	unsigned int bit;
	printk("fwq gpioapi set_direction pin=%d,dir_input=%d\n",offset,input);
	...
	if(pctl->devdata->mt_set_gpio_dir){
		...
		printk("fwq pinctrl set_direction pin=%d,dirinput=%d base not ready\n",offset,input);
	}
...
static void mtk_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	printk("fwq pinctrl set gpio out pin=%d,value=%d\n",offset,value);
	if(pctl->devdata->mt_set_gpio_out){
		mt_set_gpio_out(offset|0x80000000,value);
	}
	else{
		printk("fwq pinctrl set gpio out base not ready pin=%d,value=%d\n",offset,value);
	}
		...
		int value, enum pin_config_param param)
{
	...
	printk("fwq pinctrl mtk_pconf_set_smt pin=%d,value=%d,para=%d\n",pin,value,param);
		...
		int value, enum pin_config_param param)
{
	int ret;
    //only set ies
	printk("fwq pinctrl set ies &smt pin=%d,value=%d,para=%d\n",pin,value,param);
...
{
   printk("fwq pinctrl set mtk_pconf_set_direction pin=%d,value=%d,para=%d\n",pin,value,param);
   if(pctl->devdata->mt_set_gpio_dir){
		...
		printk("fwq pinctrl set_direction pin=%d,dir=%d base not ready\n",pin,value);
   }
}
		...
		unsigned int pin, unsigned char driving)
{
	...
	printk("fwq pinctrl set driving pin=%d,driving=%x\n",pin,driving);
		...
		unsigned int pin, bool enable, bool isup, unsigned int arg)
{
	...
	printk("fwq pinctrl set pullen & pull_select en=%d pin=%d,isup=%d\n",enable,pin,isup);
	...
	if (pctl->devdata->mt_set_gpio_pull_enable) {
		...
		printk("pinctrl gpin pull enable base not ready\n" );
	}
	...
	if (pctl->devdata->mt_set_gpio_pull_select) {
		...
		printk("pinctrl gpin pull select base not ready\n" );
	}
...
{
	struct mtk_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
	printk("fwq pinconfig param=%d, arg=%d \n",param,arg );
		...
		unsigned long pin, unsigned long mode)
{
	...
	int ret;
	struct mtk_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
	printk("fwq pinctrl set mode pin=%ld,mode=%ld\n",pin,mode);
    ...
    printk("fwq reg_addr=%x\n",reg_addr);
	bit = pin % MAX_GPIO_MODE_PER_REG;
	printk("fwq bit=%x\n",bit);
	mask <<= (GPIO_MODE_BITS * bit);
	val = (mode << (GPIO_MODE_BITS * bit));

	printk("fwq pinctrl set mode done\n");
	return regmap_update_bits(mtk_get_regmap(pctl, pin),
			reg_addr, mask, val);
					...
					unsigned offset, int value)
{
    ...
    printk("fwq pinctrl set out pin=%d,out=%d\n",offset,value);
...
static int mtk_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	struct mtk_pinctrl *pctl = dev_get_drvdata(chip->dev);
	printk("fwq pinctrl get_direction pin=%d\n",offset);
	...
	if(pctl->devdata->mt_get_gpio_dir){
		...
		printk("fwq pinctrl get dir pin=%d,base not ready\n",offset);
	}
...
static int mtk_gpio_get(struct gpio_chip *chip, unsigned offset)
{
    ...
    if(pctl->devdata->mt_get_gpio_out==NULL || pctl->devdata->mt_get_gpio_in == NULL){
		printk("fwq pinctrl mtk_gpio_get base not ready  \n");
		printk("fwq pinctrl mtk_gpio_get out=%p \n",pctl->devdata->mt_get_gpio_out);
		printk("fwq pinctrl mtk_gpio_get in=%p \n",pctl->devdata->mt_get_gpio_in);
		return 0;
	...
	return value;
	/*
	printk("fwq pinctrl get gpio out pin=%d\n",offset);
	if(pctl->devdata->mt_get_gpio_out){
		dout = mt_get_gpio_out(offset|0x80000000);
	}
	else{
		printk("fwq pinctrl set gpio out base not ready pin=%d\n",offset);
	}
	return dout;
	*/
	...
	unsigned debounce)
{
   ...
   printk("fwq pinctrl mtk_gpio_set_debounce pin=%d\n",offset);
