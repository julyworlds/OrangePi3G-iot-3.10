...
#define HCI_STP_READY_TIMEOUT msecs_to_jiffies(2000) /* 2 seconds */

/*******************************************************************************
*                            P U B L I C   D A T A
********************************************************************************
*/
unsigned int gDbgLevelCK = BT_LOG_INFO;
...
#define INTERNAL_BT_CFG_FILE        "/etc/firmware/BT.cfg"
#define CUSTOM_BT_CFG_FILE          "/etc/firmware/nvram/BT_Addr"
...
static struct hci_stp_init_cmd init_table_6628[] =
{
...
#define BT_LOUD_FUNC(fmt, arg...)   if(gDbgLevelCK >= BT_LOG_LOUD){printk(PFX "[L]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_DBG_FUNC(fmt, arg...)    if(gDbgLevelCK >= BT_LOG_DBG){printk(PFX "[D]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_INFO_FUNC(fmt, arg...)   if(gDbgLevelCK >= BT_LOG_INFO){printk(PFX "[I]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_WARN_FUNC(fmt, arg...)   if(gDbgLevelCK >= BT_LOG_WARN){printk(PFX "[W]%s:"  fmt, __FUNCTION__ ,##arg);}
#define BT_ERR_FUNC(fmt, arg...)    if(gDbgLevelCK >= BT_LOG_ERR){printk(PFX "[E]%s:"   fmt, __FUNCTION__ ,##arg);}
#define BT_TRC_FUNC(f)              if(gDbgLevelCK >= BT_LOG_LOUD){printk(PFX "[T]%s:%d\n", __FUNCTION__, __LINE__);}
...
static void hci_stp_tx_tasklet_func(unsigned long data) {
    ...
    // if (unlikely(hu != hdev->driver_data)) {//CKT sunqidong modify for krillin kernel
    if (unlikely(hu != hci_get_drvdata(hdev))) {
        BT_ERR_FUNC("hu(0x%p) != hci_get_drvdata(hdev)(0x%p)\n",
            hu, hci_get_drvdata(hdev));
        BUG_ON(hu != hci_get_drvdata(hdev));
        return;
    }
...
void hci_stp_dev_init_rx_cb (const UINT8 *data, INT32 count)
{
    ...
    // if (unlikely(!hdev->driver_data)) {//CKT sunqidong modify for krillin kernel
    if (unlikely(!hci_get_drvdata(hdev))) {
        BT_ERR_FUNC("null hdev->driver_data!\n");
        return;
    }

    /* get hci_stp from global variable */
    // hu = (struct hci_stp *)hdev->driver_data;//CKT sunqidong modify for krillin kernel
    hu = hci_get_drvdata(hdev);
    idx = hu->init_cmd_idx;
...
static void hci_stp_dev_init_work (struct work_struct *work)
{
    ...
    BT_DBG_FUNC("[tx power offset %02x %02x %02x]\n",
     ...
     if (0 == memcmp(cfg.addr, sDefaultCfg.addr, 6)){
                ...
                unsigned long randNum1;
                    /* eFUSE address default value, enable auto-gen */
                    BT_DBG_FUNC("eFUSE address default value, enable auto-gen!\n");
                    get_random_bytes(&randNum1, sizeof(unsigned long));
                    BT_DBG_FUNC("Get random number: %lu\n", randNum1);

                    cfg.addr[0] = (((randNum1>>24 | randNum1>>16) & (0xFE)) | (0x02));
                    cfg.addr[1] = ((randNum1>>8) & 0xFF);
                    cfg.addr[5] = (randNum1 & 0xFF);
             #endif
       }
...
static int hci_stp_open(struct hci_dev *hdev)
{
    ...
    if (likely(mtk_wcn_stp_is_ready())) {
        ...
        BT_INFO_FUNC("Marked HCI as running\n");
        ...
        BT_INFO_FUNC("hci_stp_open done\n");
...
static int hci_stp_flush(struct hci_dev *hdev)
{
    ...
    BT_DBG_FUNC("start\n");
    //if (!hdev || !hdev->driver_data) {//CKT sunqidong modify for krillin kernel
    if (!hdev || !hci_get_drvdata(hdev)) {
        BT_WARN_FUNC("invalid hdev(0x%p) or drv data(0x%p)\n",
            hdev, (hdev) ? hci_get_drvdata(hdev) : hdev);
        return -EFAULT;
    }
...
static void hci_stp_destruct(struct hci_dev *hdev)
{
    ...
    BT_DBG_FUNC("%s\n", hdev->name);
    // hci_stp_tx_deinit((struct hci_stp *)hdev->driver_data);//CKT sunqidong modify for krillin kernel
    hci_stp_tx_deinit(hci_get_drvdata(hdev));
    kfree(hci_get_drvdata(hdev));
}
...
#endif
    //hdev->driver_data = hu;//CKT sunqidong modify for krillin kernel
    hci_set_drvdata(hdev, hu);

    BT_DBG_FUNC("hdev->driver_data 0x%08x\n", (int)hci_get_drvdata(hdev));
    hdev->open  = hci_stp_open;
    hdev->close = hci_stp_close;
    hdev->flush = hci_stp_flush;
    hdev->send  = hci_stp_send_frame;
    // hdev->destruct = hci_stp_destruct;//CKT sunqidong modify for krillin kernel

    //hdev->owner = THIS_MODULE;//CKT sunqidong modify for krillin kernel

    BT_DBG_FUNC("HCI_QUIRK_RESET_ON_CLOSE\n");
...
static int __exit hci_stp_exit(void)
{
    struct hci_stp *hu = 0;

    /* If we haven't registered the device yet there is nothing to cleanup */
    if (!hdev)
        return 0;
...
int ckt_hci_init(void)
{
    printk("FUNC ckt_hci_init\n");
    return hci_stp_init();
}

int ckt_hci_exit(void)
{
    printk("FUNC ckt_hci_exit\n");
    return hci_stp_exit();
}

EXPORT_SYMBOL(ckt_hci_init);
EXPORT_SYMBOL(ckt_hci_exit);
