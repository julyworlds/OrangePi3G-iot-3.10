...
#define ENTER_FUNC(lv)      do { if (lv & FUNC_LV_MASK) clk_info(">> %s()\n", __FUNCTION__); } while(0)
#define EXIT_FUNC(lv)       do { if (lv & FUNC_LV_MASK) clk_info("<< %s():%d\n", __FUNCTION__, __LINE__); } while(0)
#else
...
#define clk_readl(addr) \
    ((FUNC_LV_REG_ACCESS & FUNC_LV_MASK) ? clk_info("clk_readl("HEX_FMT") @ %s():%d\n", (addr), __FUNCTION__, __LINE__) : 0, 0)

#define clk_writel(addr, val)   \
    do { if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) clk_info("clk_writel("HEX_FMT", "HEX_FMT") @ %s():%d\n", (addr), (val), __FUNCTION__, __LINE__); } while(0)

#define clk_setl(addr, val) \
    do { if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) clk_info("clk_setl("HEX_FMT", "HEX_FMT") @ %s():%d\n", (addr), (val), __FUNCTION__, __LINE__); } while(0)

#define clk_clrl(addr, val) \
    do { if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) clk_info("clk_clrl("HEX_FMT", "HEX_FMT") @ %s():%d\n", (addr), (val), __FUNCTION__, __LINE__); } while(0)
...
#define clk_writel(addr, val)   \
    do { unsigned int value; if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) clk_info("clk_writel("HEX_FMT", "HEX_FMT") @ %s():%d\n", (addr), (value = val), __FUNCTION__, __LINE__); mt65xx_reg_sync_writel((value), (addr)); } while(0)
...
int mt_disable_clock(enum cg_clk_id id, const char *name)
{
...
#if 1   // XXX: debug for MT_CG_APDMA_SW_CG
    {
        ...
        if (MT_CG_APDMA_SW_CG == id)
        {
            ...
            if (!clk_cnt_rec[i].cnt) clk_err("Assert @ %s\n", clk_cnt_rec[i].name);
                                          ...
                                          )
{
    int i;
    int err = 0;
    unsigned long expired = (unsigned long)sched_clock() + 100 * 1000;
    ...
    if (state == STA_POWER_DOWN)
    {
        ...
        if (0 != sys->bus_prot_mask)
        {
            ...
            {
                if (time_after((unsigned long)sched_clock(), expired))
                {
                    WARN_ON(1);
                    break;
                }
...
static int clk_test_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "[CLK_MUX_SEL]="HEX_FMT"\n", clk_readl(CLK_MUX_SEL));
...
static int clk_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[32];
    int len = 0;
...
static int pll_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[32];
    int len = 0;
...
static int pll_fsel_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[32];
    int len = 0;
...
static int subsys_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[32];
    int len = 0;
...
static int udelay_test_read(struct seq_file *m, void *v)
{
    ENTER_FUNC(FUNC_LV_BODY);

	seq_printf(m, "\n********** udelay_test help *********\n");
	seq_printf(m, "test udelay:  echo delay > /proc/clkmgr/udelay_test\n");
...
static int udelay_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[32];
    int len = 0;
...
static int golden_test_read(struct seq_file *m, void *v)
{
    ...
    ENTER_FUNC(FUNC_LV_BODY);
		...
		if (!strcmp(_golden.func, __FUNCTION__) && (_golden.line == 0))
            {
...
static int golden_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[256];
    int len = 0;
    ...
    else if (sscanf(desc, "filter %63s %d", _golden.func, &_golden.line) == 2) // XXX: 63 = sizeof(_golden.func) - 1
    {
    }
    // set filter (func)
    else if (sscanf(desc, "filter %63s", _golden.func) == 1) // XXX: 63 = sizeof(_golden.func) - 1
    {
        _golden.line = 0;
    }
    // set golden setting (mixed mode)
    else if (sscanf(desc, "0x%x 0b%63s", &addr, cmd) == 2) // XXX: 63 = sizeof(cmd) - 1
    {
    ...
    else if (sscanf(desc, "%63s", cmd) == 1) // XXX: 63 = sizeof(cmd) - 1
    {
        if (!strcmp(cmd, "enable"))
        {
...
static int proc_udelay_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, udelay_test_read, NULL);
}
static const struct file_operations udelay_test_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_pll_fsel_open,
    .read  = seq_read,
...
#if defined(CONFIG_MT_ENG_BUILD)
static int proc_golden_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, golden_test_read, NULL);
}
static const struct file_operations golden_test_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_pll_fsel_open,
    .read  = seq_read,
    .write = golden_test_write,
...
void mt_clkmgr_debug_init(void)
{
    ...
    entry = proc_create("udelay_test", S_IRUGO | S_IWUSR, clkmgr_dir, &udelay_test_proc_fops);
