...
#define ENTER_FUNC(lv)      do { if (lv & FUNC_LV_MASK) pr_debug(TAG ">> %s()\n", __FUNCTION__); } while(0)
#define EXIT_FUNC(lv)       do { if (lv & FUNC_LV_MASK) pr_debug(TAG "<< %s():%d\n", __FUNCTION__, __LINE__); } while(0)
#else
...
#define spm_read(addr) \
    ((FUNC_LV_REG_ACCESS & FUNC_LV_MASK) ? pr_debug(TAG "spm_read(0x%08d) @ %s():%d\n", (addr), __FUNCTION__, __LINE__) : 0, 0)

#define spm_write(addr, val)   \
    do { if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) pr_debug(TAG "spm_write(0x%08d, 0x%08d) @ %s():%d\n", (addr), (val), __FUNCTION__, __LINE__); } while(0)
#else
#define spm_read(addr)          ((FUNC_LV_REG_ACCESS & FUNC_LV_MASK) ? pr_debug(TAG "spm_read(0x%08d) @ %s():%d\n", (addr), __FUNCTION__, __LINE__) : 0, (*(volatile U32 *)(addr)))
#define spm_write(addr, val)    do { if (FUNC_LV_REG_ACCESS & FUNC_LV_MASK) pr_debug(TAG "spm_write(0x%08d, 0x%08d) @ %s():%d\n", (addr), (val), __FUNCTION__, __LINE__); mt65xx_reg_sync_writel((val), (addr)); } while(0)
