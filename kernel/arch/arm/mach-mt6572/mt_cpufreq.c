...
int cpu_down(unsigned int cpu) { return 0; }
#endif
...
static struct mt_cpu_power_info mt_cpu_golden_power[] = {
    ...
    {.cpufreq_khz = DVFS_D1, .cpufreq_volt = DVFS_V1, .cpufreq_ncpu = 1, .cpufreq_power = 383 },
    {.cpufreq_khz = DVFS_D2, .cpufreq_volt = DVFS_V1, .cpufreq_ncpu = 1, .cpufreq_power = 363 },
    {.cpufreq_khz = DVFS_F2, .cpufreq_volt = DVFS_V1, .cpufreq_ncpu = 2, .cpufreq_power = 362 },
    {.cpufreq_khz = DVFS_D3, .cpufreq_volt = DVFS_V1, .cpufreq_ncpu = 1, .cpufreq_power = 343 },
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
    ...
    for(i = num; i < MAX_SPM_PMIC_TBL; i++){
        if(spm_pmic_config[i].cpufreq_volt > DVFS_MIN_VCORE){
        	  cpufreq_info("PTPOD: freq need PTPOD: %d\n", spm_pmic_config[i].cpufreq_khz);
        ...
        if(spm_pmic_config[i].cpufreq_khz == freq){
            switch(spm_pmic_config[i].tbl_idx){
                ...
                case 5:
                    ...
                    break;
                default:
                    cpufreq_dbg("There is no entry [%d] in spm table for ptpod", spm_pmic_config[i].tbl_idx);
                    break;
            }
            // set spm_pmic_config & mt_cpu_freqs.cpufreq_volt up-to-date
            ...
            cpufreq_dbg("Set spm_pmic_config[%d].cpufreq_volt = %d", i, spm_pmic_config[i].cpufreq_volt);
            //set vcore again to make sure value take effect
            mt_vcore_freq_volt_set(mt_cpu_freqs[g_cur_OPP].cpufreq_volt, mt_cpu_freqs[g_cur_OPP].cpufreq_khz);
        }
    }
    ...
    for(i = 0; i < g_cpufreq_power_tbl_num; i++){
        if((mt_cpu_power[i].cpufreq_khz == mt_cpu_freqs[idx].cpufreq_khz) && (mt_cpu_power[i].cpufreq_ncpu == cpu_num)){
            if(mt_cpu_power[i].cpufreq_power > mt_cpu_power[g_max_power_OPP].cpufreq_power){
                cpufreq_dbg("%d > %d", mt_cpu_power[i].cpufreq_power, mt_cpu_power[g_max_power_OPP].cpufreq_power);
...
void mt_dvfs_power_dispatch(void)
{
    ...
    if(total_power_budget > (cpu_power_budget + gpu_power_budget)){
        ...
        cpufreq_dbg("cpu_loading: %d , gpu_loading = %d\n", cpu_loading, gpu_loading);
        cpufreq_dbg("cpu_power_budget: %d , gpu_power_budget = %d\n", cpu_power_budget, gpu_power_budget);
...
void set_dvfs_thermal_limit(unsigned int limited_power)
{
    ...
    if(g_limited_power == 0){
        ...
        cpufreq_dbg("[MIN] cpu_power_budget: %d , gpu_power_budget = %d\n", mt_cpufreq_min_power(), mt_gpufreq_min_power());
...
{
    if (!mutex_trylock(&power_mutex)) {
        cpufreq_info("@%s: power dispatching, skip this round!\n", __func__);
        return;
    }
...
int CPU_freq_output()
{
    ...
    {
        //- check error code
        cpufreq_dbg("CPU_freq_output: freq meter kick fail!! \n");
        return -1;
    }
    else
    {
        ...
        {
            //- check error code
            cpufreq_dbg("CPU_freq_output: freqmeter get result fail!! \n");
            return -1;
        }
        else
        {
...
int GPU_freq_output()
{
    ...
    {
        // check error code
        cpufreq_dbg("GPU_freq_output: freq meter kick fail!! \n");
        return -1;
    }
    else
    {
        ...
        {
            // check error code
            cpufreq_dbg("GPU_freq_output: freqmeter get result fail!! \n");
            return -1;
        }
    else
    {
...
int mt_cpufreq_state_set(int enabled)
{
    ...
    if (enabled)
    {
        if (!mt_cpufreq_pause)
        {
            cpufreq_info("cpufreq already enabled\n");
            return 0;
        }
        ...
        **************/
        ...
        cpufreq_info("enable DVFS: g_dvfs_disable_count = %d\n", g_dvfs_disable_count);
        ...
        ************************************************/
        ...
        {
            cpufreq_info("someone still disable cpufreq, cannot enable it\n");
        }
    }
    else
    {
        ...
        ***************/
        g_dvfs_disable_count++;
        cpufreq_info("disable DVFS: g_dvfs_disable_count = %d\n", g_dvfs_disable_count);

        if (mt_cpufreq_pause)
        {
            cpufreq_info("cpufreq already disabled\n");
            return 0;
        }
...
void mt_vcore_freq_volt_set(unsigned int target_volt, unsigned int target_freq)
{
	  ...
	  if(target_freq == 0 || target_volt <= DVFS_MIN_VCORE){
		    ...
		    {
		        case DVFS_V0:
		            cpufreq_dbg("switch vcore to fix DVS0: %d mV\n", DVFS_V0);
		            spm_dvfs_ctrl_volt(0);
		            g_vcore = target_volt;
		            break;
		        case DVFS_V1:
		            cpufreq_dbg("switch vcore to fix DVS1: %d mV\n", DVFS_V1);
...
static void mt_cpufreq_set(unsigned int old_OPP, unsigned int new_OPP)
{
    ...
    if( target_freq> PLL_MAX_FREQ || target_freq < PLL_MIN_FREQ){
        cpufreq_err("set_armpll_freq: freq [%d] out of range\n", target_freq);
        return;
    }
    else
    {
        ...
        {
            cpufreq_warn("set_armpll_freq: unsupport frequency!!\n");
        }
    }
    ...
    g_cur_OPP = new_OPP;
    cpufreq_dbg("ARMPLL_CON0 = 0x%x, ARMPLL_CON1 = 0x%x, current_freq = %d\n", DRV_Reg32(ARMPLL_CON0), DRV_Reg32(ARMPLL_CON1), mt_cpu_freqs[g_cur_OPP].cpufreq_khz);
    cpufreq_dbg("[CPU freq]: %d\n", CPU_freq_output());
}
    ...
    for (i = g_max_power_OPP; i < (g_cpufreq_power_tbl_num); i++)
    {
        ...
        if(mt_cpu_power[i].cpufreq_ncpu == cpu_num)
        {
                ...
                if( mt_cpu_power[i].cpufreq_khz >= mt_cpu_freqs[OPP_index].cpufreq_khz ){
                    ...
                    cpufreq_dbg("[verified] target_freq = %d, ncpu = %d\n", mt_cpu_freqs[OPP_index].cpufreq_khz, cpu_num);
                    return OPP_index;
                }
        }
    }
    cpufreq_warn("Can't find suitable OPP for thermal throttling!\n");
    cpufreq_warn("g_max_power_OPP = %d, freq = %d, ncpu = %d\n", g_max_power_OPP, mt_cpu_freqs[g_cur_OPP].cpufreq_khz / 1000, cpu_num);
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    if(g_limited_power != 0){
        ...
        if (!mutex_trylock(&power_mutex)) {
            ...
            if (num_online_cpus() > g_limited_max_ncpu)
            {
                ...
                {
                    cpufreq_dbg("turn off CPU%d due to thermal protection\n", (i - 1));
                    cpu_down((i - 1));
                }
            }
#endif
        ...
        if (new_OPP_idx < 0) {
            cpufreq_info("@%s: skip this round due to CPU off not complete!\n", __func__);
            mutex_unlock(&power_mutex);
            return 0;
        }
        ...
        mutex_unlock(&power_mutex);
        cpufreq_dbg("CPU freq after thermal verified: %d MHZ\n", mt_cpu_freqs[new_OPP_idx].cpufreq_khz / 1000);
    }
    ...
    ***************************************/
    ...
    {
        cpufreq_err("Error!! Cannot find corresponding voltage at %d Mhz\n", freqs.new / 1000);
        return 0;
    }*/
...
static int mt_cpufreq_init(struct cpufreq_policy *policy)
{
    ...
    **********************************************/
    ...
    cpufreq_info("g_cpufreq_get_ptp_level = %d\n", g_cpufreq_get_ptp_level);
...
void mt_cpufreq_return_default_DVS_by_ptpod(void)
{
    ...
    */
    cpufreq_info("mt_cpufreq return default DVS by ptpod\n");
}
...
void mt_cpufreq_enable_by_ptpod(void)
{
    ...
    cpufreq_info("mt_cpufreq enabled by ptpod\n");
}
...
/*unsigned int mt_cpufreq_disable_by_ptpod(void)
{
    ...
    cpufreq_info("mt_cpufreq disabled by ptpod, limited freq. at %d\n", DVFS_F2);
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
    ...
    if (limited_power == 0) // no limitation
    {
        ...
        cpufreq_dbg("[thermal] max_freq = %d, max_ncpu = %d\n", g_limited_max_freq, g_limited_max_ncpu);
    }
    else
    {
        while (ncpu)
        ...
        if (!found)
        {
            cpufreq_warn("thermal limit fail, not found suitable DVFS OPP\n");
        }
        else
        {
            cpufreq_dbg("[thermal] max_freq = %d, max_ncpu = %d\n", g_limited_max_freq, g_limited_max_ncpu);
...
****************************/
static int mt_cpufreq_state_read(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int len = 0;
    char *p = buf;
...
*************************************/
static ssize_t mt_cpufreq_state_write(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
    int enabled = 0;
    ...
    if (sscanf(buffer, "%d", &enabled) == 1)
    {
        ...
        {
            cpufreq_err("bad argument!! argument should be \"1\" or \"0\"\n");
        }
    }
    else
    {
...
*****************************/
static int mt_cpufreq_limited_power_read(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int len = 0;
    char *p = buf;
...
***********************************/
static ssize_t mt_cpufreq_limited_power_write(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
    ...
    {
        cpufreq_err("bad argument!! please provide the maximum limited power\n");
    }
...
****************************/
static int mt_cpufreq_debug_read(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int len = 0;
    char *p = buf;
...
************************/
static ssize_t mt_cpufreq_debug_write(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
    int debug = 0;
    ...
    if (sscanf(buffer, "%d", &debug) == 1)
    {
        ...
        {
            cpufreq_err("bad argument!! should be 0 or 1 [0: disable, 1: enable]\n");
        }
    }
    else
    {
...
****************************/
static int mt_cpufreq_power_dump_read(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    ...
    if(0){
        ...
        for (i = 0; i < (g_cpufreq_power_tbl_num); i++)
        {
            cpufreq_info("mt_cpu_power[%d].cpufreq_khz = %d\n", i, mt_cpu_power[i].cpufreq_khz);
            cpufreq_info("mt_cpu_power[%d].cpufreq_ncpu = %d\n", i, mt_cpu_power[i].cpufreq_ncpu);
            cpufreq_info("mt_cpu_power[%d].cpufreq_power = %d\n", i, mt_cpu_power[i].cpufreq_power);
        }
        pwrap_read(VPROC_VOSEL_CTRL, &pmic_setting);
        cpufreq_info("VPROC_VOSEL_CTRL(0x%x) = %d\n", VPROC_VOSEL_CTRL, pmic_setting);

        for(i = 0; i < MAX_SPM_PMIC_TBL; i++){
            cpufreq_info("spm_pmic_config[%d].tbl_idx = %d\n", i, spm_pmic_config[i].tbl_idx);
            cpufreq_info("spm_pmic_config[%d].cpufreq_khz = %d\n", i, spm_pmic_config[i].cpufreq_khz);
            cpufreq_info("spm_pmic_config[%d].cpufreq_volt = %d\n", i, spm_pmic_config[i].cpufreq_volt);
        }
        for(i = 0; i < mt_cpu_freqs_num; i++) {
            cpufreq_info("mt_cpu_freqs[%d].cpufreq_khz = %d\n", i, mt_cpu_freqs[i].cpufreq_khz);
            cpufreq_info("mt_cpu_freqs[%d].cpufreq_volt = %d\n", i, mt_cpu_freqs[i].cpufreq_volt);
        }
...
static int mt_cpufreq_freq_read(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int i, len;
    char *p = buf;
...
static ssize_t mt_cpufreq_freq_write(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
    unsigned int freq;
    struct cpufreq_policy *policy;
...
static int mt_cpufreq_get_power_budget(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int /* i, */ len;
    char *p = buf;
...
static ssize_t mt_cpufreq_set_power_budget(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
    unsigned int power_budget;
    if (sscanf(buffer, "%u", &power_budget) == 1){

        cpufreq_info("Set PowerBudget: [%u]\n", power_budget);
...
static int mt_cpufreq_get_armpll_dds(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int /* i, */ len;
    char *p = buf;
...
static ssize_t mt_cpufreq_set_armpll_dds(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
...
static int mt_cpufreq_get_usb_raise(struct file *filep, char __user *buf, size_t count, loff_t *offp)
{
    int /* i, */ len;
    char *p = buf;
...
static ssize_t mt_cpufreq_set_usb_raise(struct file *filep, const char __user *buffer, size_t count, loff_t *offp)
{
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
    ...
    cpufreq_info("PMIC_WRAP_DVFS_WDATA0: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA0));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA1: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA1));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA2: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA2));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA3: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA3));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA4: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA4));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA5: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA5));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA6: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA6));
    cpufreq_info("PMIC_WRAP_DVFS_WDATA7: 0x%x\n", dvfs_read(PMIC_WRAP_DVFS_WDATA7));

    mt_cpufreq_ready = true;

    cpufreq_info("mediatek cpufreq initialized\n");
...
{
    cpufreq_info("mt_cpufreq_suspend\n");
...
{
    cpufreq_info("mt_cpufreq_resume\n");
...
static const struct file_operations cpufreq_debug_ops = {
        .read = mt_cpufreq_debug_read,
        .write = mt_cpufreq_debug_write,
};

static const struct file_operations cpufreq_limited_power_ops = {
        .read = mt_cpufreq_limited_power_read,
        .write = mt_cpufreq_limited_power_write,
};

static const struct file_operations cpufreq_state_ops = {
        .read = mt_cpufreq_state_read,
        .write = mt_cpufreq_state_write,
};

static const struct file_operations cpufreq_power_dump_ops = {
        .read = mt_cpufreq_power_dump_read,
};

static const struct file_operations cpufreq_cur_freq_ops = {
        .read = mt_cpufreq_freq_read,
        .write = mt_cpufreq_freq_write,
};

static const struct file_operations armpll_dds_ops = {
        .read = mt_cpufreq_get_armpll_dds,
        .write = mt_cpufreq_set_armpll_dds,
};

static const struct file_operations power_budget_ops = {
        .read = mt_cpufreq_get_power_budget,
        .write = mt_cpufreq_set_power_budget,
};

static const struct file_operations usb_raise_freq_ops = {
        .read = mt_cpufreq_get_usb_raise,
        .write = mt_cpufreq_set_usb_raise,
};
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    {
            mt_entry = proc_create("cpufreq_debug", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_debug_ops);

            mt_entry = proc_create("cpufreq_limited_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_limited_power_ops);

            mt_entry = proc_create("cpufreq_state", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_state_ops);

            mt_entry = proc_create("cpufreq_power_dump", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_power_dump_ops);

            mt_entry = proc_create("cpufreq_cur_freq", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &cpufreq_cur_freq_ops);

            mt_entry = proc_create("armpll_dds", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &armpll_dds_ops);

            mt_entry = proc_create("power_budget", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &power_budget_ops);

            mt_entry = proc_create("usb_raise_freq", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &usb_raise_freq_ops);
    }

    ret = platform_driver_register(&mt_cpufreq_pdrv);
    if (ret)
    {
        cpufreq_err("failed to register cpufreq driver\n");
        return ret;
    }
    else
    {
        cpufreq_info("cpufreq driver registration done\n");
        return 0;
    }
}
