...
int mt_ptp_thread_handler(void *unused)
{
    ...
    do
    {
        ...
        if( (temperature > 0x4b) && (temperature < 0xd3) ){
            ...
            ptp_print("PTP_LOG: (%d) - (0x%d, 0x%d, 0x%d, 0x%d, 0x%d, 0x%d, 0x%d, 0x%d) - (%d, %d, %d, %d, %d, %d, %d, %d)\n", \
                    (temperature + 25)*1000, \
...
{
	  if(freq != 0){
	  	  ptp_isr_print("set voltage: 0x%u frequency: %d\n", volt, freq);
...
irqreturn_t PTP_ISR(int irq, void *dev_id)
{
    ...
    if( PTPINTSTS == 0x1 ) // PTP init1 or init2
    {
        if((ptpen & 0x7) == 0x1){ // init1
            ...
            ptp_isr_print("Init2 PTP_V_0 = 0x%u\n", PTP_V[0]);
            ptp_isr_print("Init2 PTP_V_1 = 0x%u\n", PTP_V[1]);
            ptp_isr_print("Init2 PTP_V_2 = 0x%u\n", PTP_V[2]);
            ptp_isr_print("Init2 PTP_V_3 = 0x%u\n", PTP_V[3]);
            ptp_isr_print("Init2 PTP_V_4 = 0x%u\n", PTP_V[4]);
            ptp_isr_print("Init2 PTP_V_5 = 0x%u\n", PTP_V[5]);
            ptp_isr_print("Init2 PTP_V_6 = 0x%u\n", PTP_V[6]);
            ptp_isr_print("Init2 PTP_V_7 = 0x%u\n", PTP_V[7]); 
            ...
            ptp_isr_print("====================================================\n");
            ptp_isr_print("PTP error_0 (0x%d) : PTPINTSTS = 0x%d\n", ptpen, PTPINTSTS);
            ptp_isr_print("====================================================\n");
    ...
    {   
        ...
        if( (temperature > 0x4b) && (temperature < 0xd3) ){
            ...
            for(i = 0; i < MAX_OPP_NUM; i++){
                ...
                if(PTP_Change[i] != 0){
                    ptp_isr_print("MON_Volt Changed. PTP_MON_V[%d] = 0x%u\n", i, PTP_MON_V[i]);
                    PTP_set_ptp_volt(PTP_MON_V[i], CPU_Freq[i]);
                }
            }
            
            ptp_isr_print("TEMPSPARE1 = 0x%d\n", ptp_read(TEMPSPARE1));
    ...
    else // PTP error
    {
        ...
        {
            ptp_isr_print("====================================================\n");
            ptp_isr_print("PTP error_1 error_2 (0x%d) : PTPINTSTS = 0x%d\n", ptpen, PTPINTSTS);
            ptp_isr_print("PTP_SMSTATE0 (0x%d) = 0x%d\n", PTP_SMSTATE0, ptp_read(PTP_SMSTATE0) );
            ptp_isr_print("PTP_SMSTATE1 (0x%d) = 0x%d\n", PTP_SMSTATE1, ptp_read(PTP_SMSTATE1) );
            ptp_isr_print("====================================================\n");
        ...
        {
            ptp_isr_print("====================================================\n");
            ptp_isr_print("PTP error_m (0x%d) : PTPINTSTS = 0x%d\n", ptpen, PTPINTSTS);
            ptp_isr_print("PTP_SMSTATE0 (0x%d) = 0x%d\n", PTP_SMSTATE0, ptp_read(PTP_SMSTATE0) );
            ptp_isr_print("PTP_SMSTATE1 (0x%d) = 0x%d\n", PTP_SMSTATE1, ptp_read(PTP_SMSTATE1) );
            ptp_isr_print("PTP_TEMP (0x%d) = 0x%d\n", PTP_TEMP, ptp_read(PTP_TEMP) );
            
            ptp_isr_print("PTP_TEMPMSR0 (0x%d) = 0x%d\n", TEMPMSR0, ptp_read(TEMPMSR0) );
            ptp_isr_print("PTP_TEMPMSR1 (0x%d) = 0x%d\n", TEMPMSR1, ptp_read(TEMPMSR1) );
            ptp_isr_print("PTP_TEMPMSR2 (0x%d) = 0x%d\n", TEMPMSR2, ptp_read(TEMPMSR2) );
            ptp_isr_print("PTP_TEMPMONCTL0 (0x%d) = 0x%d\n", TEMPMONCTL0, ptp_read(TEMPMONCTL0) );
            ptp_isr_print("PTP_TEMPMSRCTL1 (0x%d) = 0x%d\n", TEMPMSRCTL1, ptp_read(TEMPMSRCTL1) );
            ptp_isr_print("====================================================\n");
...
static void ptp_init2(void)
{
    ...
    ptp_print("PTP_Init_value.DCVOFFSETIN = 0x%u\n", PTP_Init_value.DCVOFFSETIN);
    ptp_print("PTP_Init_value.AGEVOFFSETIN = 0x%u\n", PTP_Init_value.AGEVOFFSETIN);
...
static int ptp_dump_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    p += sprintf(p, "(0x%u, 0x%u, 0x%u, 0x%u)\n", val_0, val_1, val_2, val_3);
...
static int ptp_cur_volt_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    {
        p += sprintf(p, "current voltage: (0x%u)\n", rdata);
    }
    else
    {
