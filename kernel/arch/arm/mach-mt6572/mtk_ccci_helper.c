...
#ifndef CONFIG_MD1_SIZE
#define MD1_MEM_SIZE    (18*1024*1024)
#else
#define MD1_MEM_SIZE    CONFIG_MD1_SIZE
#endif

#ifndef CONFIG_MD1_SMEM_SIZE
#define CONFIG_MD1_SMEM_SIZE	(2*1024*1024)
#endif  // CONFIG_MD1_SMEM_SIZE
...
static dfo_item_t ccci_dfo_setting[] =
{
	...
	{"CONFIG_MD1_SMEM_SIZE",	CONFIG_MD1_SMEM_SIZE},
	{"MD2_SMEM_SIZE",	0},
...
static void cal_md_mem_usage(void)
{
	...
	if(get_ccci_dfo_setting("CONFIG_MD1_SMEM_SIZE", &tmp) == 0) {
		tmp = round_up(tmp, 0x200000);
	...
	if(md1_en && (md_share_mem_size[MD_SYS1]>0) && (md_resv_mem_size[MD_SYS1]>0)) {
		...
		md_share_mem_size[MD_SYS1] = CONFIG_MD1_SMEM_SIZE;
